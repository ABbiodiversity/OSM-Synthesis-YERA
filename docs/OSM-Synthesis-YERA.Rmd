...
load(here("./results/occupancy/MultiYearOccupancyOutput_2013-19.RData"))
...
df_output_occ <- OUT %>%
  map(~ data.frame(.x[1])) %>%
  map_dfr(~ rownames_to_column(.x[1:7], var = "status"), .id = "region") %>%
  gather(key = "year", value = "occupancy", Occupancy.1:Occupancy.7) %>%
...
plot5 <- df_output_occ %>%
  ...
  scale_y_continuous(breaks = seq(0, 0.6, 0.2), limits = c(0, 0.7)) +
  scale_x_continuous(breaks = seq(2013, 2019, 1)) +
  theme_bw() +
  labs(x = "Year",
