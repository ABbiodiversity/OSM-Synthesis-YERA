...
author: "Richard Hedley, Erin Bayne, Marcus Becker"
...
# YERA occupancy
df_occupy <- read_csv(here("./data/processed/yera_occupy_2013-21_new_2.csv")) |>
  mutate(ss = str_replace_all(location, "-", ":"))

# Occupancy modeling output
load(here("./results/occupancy/MultiYearOccupancyOutput_2013-21_2022-04-04.RData"))
...
```{r site-map2, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, out.width="100%", fig.height=9}

df_occ_summary <- df_occupy %>%
  mutate(yera_occupied = factor(occupied, levels = c("TRUE", "FALSE", "NOT SURVEYED"))) %>%
  mutate(region = ifelse(region == "OTHER", "EAST", region))
...
bscols(widths = c(3, 3, NA),
         ...
         # Will try to figure out a way to have a default selection.
         filter_select("year", "Survey Year", shared_ss, ~year, multiple = FALSE, selected = "2013"),
         filter_checkbox("region", "Region", shared_ss,
                  ~region)
)
...
sf_osr %>%
  ...
  addCircleMarkers(data = shared_ss, color = ~pal1(yera_occupied),
                   stroke = FALSE, fillOpacity = 1, radius = 7,
                   popup = paste("Site Code:", "<b>", df_occ_summary$location)) %>%
...
df_output <- OUT %>%
  ...
  mutate(region = case_when(
    ...
    region == "UNMINEABLE" ~ "Non-Mineable Areas",
    region == "OTHER" ~ "Other")) %>%
...
Occupancy, adjusted for imperfect detection, is estimated for each of the four regional subsets between 2013 and 2021, and the results are displayed in Figure \@ref(fig:occ-comb). Occupancy is also estimated for the full region and displayed in Figure \@ref(fig:occ-full). 
...
+ Alberta Vegetation Inventory. AVI is a photo-based digital inventory developed to identify the type, extent and conditions of vegetation. Human observers digitize polygons into a series of forest type classes that include various non-forested categories. We followed a system created by Environment and Climate Change Canada (Edmonton) to convert the underlying attributes into 37 unique classes of which nine represent wetland types.
...
map_prefhab <- sf_osr %>%
  ...
  addRasterImage(ras_prefhab, colors = pal1, opacity = 1,
                 group = "Predicted Habitat Suitability", maxBytes = 6 * 1024 * 1024) %>%
